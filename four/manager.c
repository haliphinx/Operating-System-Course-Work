...
GtkWidget * refresh0(void)
{
	GtkWidget *label;	
	char bufferf1[100000];	
...
int main( int argc,char *argv[] )
{
    ...
    gtk_window_set_title (GTK_WINDOW (window), "HX-Manager");  
    gtk_widget_set_size_request (window, 650, 450);  
    ...
    sprintf(bufferf3,"版本信息");
    sprintf(bufferl, "系统信息");
    gtk_notebook_set_current_page(GTK_NOTEBOOK(notebook),0);	/*把此页设为当前显示页*/
    ...
    frame3 = gtk_frame_new (bufferf3);
    gtk_container_set_border_width (GTK_CONTAINER (frame3), 10);
    gtk_widget_set_size_request (frame2, 500, 100);
    sprintf(bufferf3, "HX-Manager  Pro. Version\t\t\t\n\nCopyright@2016  Hx\t\t\t");
    ...
    /*5.建立作者信息选项卡*/
    sprintf(bufferf1, "作者信息");
    sprintf(bufferl, "作者信息");
...
void cpu_record_draw(GtkWidget *widget)/*cpu使用记录绘图函数*/
{
	...
	for(i=0;i<139;i++)
	{
		gdk_draw_line(canvas,gc,10+i*3,110-104*cpu_data[my_first_data%140],10+(i+1)*3,110-104*cpu_data[(my_first_data+1)%140]);
		my_first_data++;
		if(my_first_data==140)	my_first_data=0;
	}
 	color.red = 0;
	color.green = 0;
	color.blue = 0;
...
void mem_record_draw(GtkWidget *widget)	/*内存记录绘图函数*/
{
	...
	mem_first_data++;	
	if(mem_first_data==140) mem_first_data=0;
	...
	for(i=0;i<139;i++)
	{
		gdk_draw_line(canvas,gc,10+i*3,109-104*mem_data[my_first_data%140],10+(i+1)*3,109-104*mem_data[(my_first_data+1)%140]);
		my_first_data++;
		if(my_first_data==140)	my_first_data=0;
	}
 	color.red = 0;
